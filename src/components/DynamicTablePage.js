...
const DynamicTablePage = () => {
  ...
  const handleInputChange = (e) => {
    ...
    if (files && files[0]) {
      ...
      // Generate a preview URL for the selected file and store it in previews state
      ...
      {tableData.map((row, index) => (
        <Table key={index} sx={{ mb: 3 }}>
          <TableHead>
            <TableRow>
              <TableCell colSpan={2}>
                <Typography variant="h6">About Banka 1964</Typography>
              </TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {columns.map((column) => (
              <TableRow key={`${column}-${index}`}>
                <TableCell sx={{ fontWeight: 600, textTransform: 'capitalize' }}>{formatLabel(column)}</TableCell>
    ...
    <Box sx={{ p: 2 }}>
      ...
      <Box mb={2} display="flex" justifyContent="space-between" alignItems="center">
        {/* <TextField
          ...
          /> */}
        <Button variant="contained" color="primary" sx={{ borderRadius: 8 }} onClick={() => handleOpenModal()}>
          Add Record
        </Button>
      </Box>
                ...
                <TableBody>
                  {columns.map((column) => {
                    if (column !== 'id') {
                      return (
                        <TableRow key={column}>
                          <TableCell sx={{ display: 'block', width: '100%', fontWeight: 600 }}>{formatLabel(column)}</TableCell>
                          <TableCell>
                            {column.startsWith('image_') && (
                              <>
                                {previews[column] ? (
                                  <img src={previews[column]} alt="preview" width="100" />
                                ) : (
                                  formData[column] && (
                                    <img src={`${baseImagePath}/${formData[column]}`} alt="preview" width="100" />
                                  )
                                )}
                            ...
                            {column.startsWith('video_') && (
                              <>
                                {previews[column] ? (
                                  <video width="100" controls>
                                    <source src={previews[column]} type="video/mp4" />
                                    Your browser does not support the video tag.
                                  </video>
                                ) : (
                                  formData[column] && (
                                    <video width="100" controls>
                                      <source src={`${baseImagePath}/${formData[column]}`} type="video/mp4" />
                                      Your browser does not support the video tag.
                                    </video>
                                  )
                                )}
                            ...
                            {column.startsWith('pdf_') && (
                              <>
                                {previews[column] ? (
                                  <a href={previews[column]} target="_blank" rel="noopener noreferrer">
                                    View PDF
                                  </a>
                                ) : (
                                  formData[column] && (
                                    <a href={`${baseImagePath}/${formData[column]}`} target="_blank" rel="noopener noreferrer">
                                      View PDF
                                    </a>
                                  )
                                )}
                            ...
                            {!(column.startsWith('image_') || column.startsWith('video_') || column.startsWith('pdf_')) && (
                              <TextField
                                name={column}
